...
#pragma unused(vm)
    ...
    if (fiber->stackalloc >= stack_needed)
        return true;
...
static bool gravity_vm_exec(gravity_vm *vm)
{
    ...
    while (1)
    {
        INTERPRET_LOOP
        {
            ...
            // MARK: LOAD
            // MARK: LOADAT
            CASE_CODE(LOAD) : CASE_CODE(LOADAT) :
            {
                OPCODE_GET_TWO8bit_ONE10bit(inst, const uint32_t r1, const uint32_t r2, const uint32_t r3);
            ...
            execute_load_function:
                ...
                case EXEC_TYPE_SPECIAL:
                {
                    if (!closure->f->special[EXEC_TYPE_SPECIAL_GETTER])
                        RUNTIME_ERROR("Missing special getter function for property %s", VALUE_AS_CSTRING(v3));
            ...
            CASE_CODE(LOADK) :
            {
                ...
                break;
                case CPOOL_VALUE_ARGUMENTS:
                    SETVALUE(r1, (frame->args) ? VALUE_FROM_OBJECT(frame->args) : VALUE_FROM_NULL);
                    break;
                case CPOOL_VALUE_NULL:
                    SETVALUE(r1, VALUE_FROM_NULL);
                    break;
                case CPOOL_VALUE_UNDEFINED:
                    SETVALUE(r1, VALUE_FROM_UNDEFINED);
                    break;
                case CPOOL_VALUE_TRUE:
                    SETVALUE(r1, VALUE_FROM_TRUE);
                    break;
                case CPOOL_VALUE_FALSE:
                    SETVALUE(r1, VALUE_FROM_FALSE);
                    break;
                case CPOOL_VALUE_FUNC:
                    SETVALUE(r1, VALUE_FROM_OBJECT(frame->closure));
                    break;
                default:
                    RUNTIME_ERROR("Unknown LOADK index");
                    break;
                }
                DISPATCH();
            }
            ...
            CASE_CODE(LOADG) :
            {
                ...
                if (!v)
                    RUNTIME_ERROR("Unable to find object %s", VALUE_AS_CSTRING(key));
            ...
            // MARK: STORE
            // MARK: STOREAT
            CASE_CODE(STORE) : CASE_CODE(STOREAT) :
            {
                OPCODE_GET_TWO8bit_ONE10bit(inst, const uint32_t r1, const uint32_t r2, const uint32_t r3);
            ...
            execute_store_function:
                ...
                case EXEC_TYPE_INTERNAL:
                {
                    ...
                    if (!result)
                    {
                        ...
                        {
                            closure = VALUE_AS_CLOSURE(STACK_GET(r1));
                            if (closure->f->tag == EXEC_TYPE_INTERNAL)
                                r1copy = STACK_GET(rwin + 1);
                            SETVALUE(r1, r1copy);
                            reset_r1 = true;
            ...
            // MARK: COMPARISON
            CASE_CODE(LT) : CASE_CODE(GT) : CASE_CODE(EQ) : CASE_CODE(LEQ) : CASE_CODE(GEQ) : CASE_CODE(NEQ) :
            {
                ...
                else if (VALUE_ISA_INT(v2) && VALUE_ISA_INT(v3))
                {
                    ...
                    if (v2.n == v3.n)
                        SETVALUE(r1, VALUE_FROM_INT(0));
                    else
                        SETVALUE(r1, VALUE_FROM_INT((v2.n > v3.n) ? 1 : -1));
                }
                else
                {
                    // prepare function call for binary operation
...
#pragma unused(r3)
                ...
                if (VALUE_ISA_INT(v2))
                {
                    SETVALUE(r1, VALUE_FROM_INT(~v2.n));
                    DISPATCH();
                }
            ...
            CASE_CODE(JUMPF) :
            {
                ...
                if (flag)
                {
                    ...
                    if ((VALUE_ISA_BOOL(STACK_GET(r1))) && (GETVALUE_INT(STACK_GET(r1)) == 0))
                    ...
                    if (marray_size(closure->f->pvalue))
                    {
                        ...
                        while (n < closure->f->nparams)
                        {
                            if (VALUE_ISA_UNDEFINED(STACK_GET(rwin + n)))
                                SETVALUE(rwin + n, marray_get(closure->f->pvalue, n - 1));
                            ++n;
                        }
                    }
                ...
                if (v1_is_map)
                {
                    ...
                    while (r2)
                    {
                        ...
                        if (!VALUE_ISA_STRING(key))
                            RUNTIME_ERROR("Unable to build Map from a non String key");
            ...
            // MARK: - RESERVED
            CASE_CODE(RESERVED2) : CASE_CODE(RESERVED3) : CASE_CODE(RESERVED4) : CASE_CODE(RESERVED5) : CASE_CODE(RESERVED6) :
            {
...
bool gravity_vm_runclosure(gravity_vm *vm, gravity_closure_t *closure, gravity_value_t sender, gravity_value_t params[], uint16_t nparams)
{
    ...
    if (f && (f->tag == EXEC_TYPE_NATIVE) && ((!f->bytecode) || (f->ninsts == 0)))
        return true;
...
gravity_value_t gravity_vm_get(gravity_vm *vm, const char *key)
{
    if (key)
    {
        if (strcmp(key, GRAVITY_VM_GCENABLED) == 0)
            return VALUE_FROM_INT(vm->gcenabled);
        if (strcmp(key, GRAVITY_VM_GCMINTHRESHOLD) == 0)
            return VALUE_FROM_INT(vm->gcminthreshold);
        if (strcmp(key, GRAVITY_VM_GCTHRESHOLD) == 0)
            return VALUE_FROM_INT(vm->gcthreshold);
        if (strcmp(key, GRAVITY_VM_GCRATIO) == 0)
            return VALUE_FROM_FLOAT(vm->gcratio);
        if (strcmp(key, GRAVITY_VM_MAXCALLS) == 0)
            return VALUE_FROM_INT(vm->maxccalls);
        if (strcmp(key, GRAVITY_VM_MAXBLOCK) == 0)
            return VALUE_FROM_INT(vm->maxmemblock);
        if (strcmp(key, GRAVITY_VM_MAXRECURSION) == 0)
            return VALUE_FROM_INT(vm->maxrecursion);
    }
...
static bool vm_set_superclass(gravity_vm *vm, gravity_object_t *obj)
{
    ...
    if (OBJECT_ISA_CLASS(obj))
    {
        ...
        c->xdata = NULL;
        if (supername)
            real_set_superclass(vm, c, key, supername);
        gravity_hash_iterate(c->htable, vm_set_superclass_callback, vm);
    }
    else if (OBJECT_ISA_FUNCTION(obj))
    {
        ...
        if (f->tag == EXEC_TYPE_NATIVE)
        {
            ...
            for (size_t i = 0; i < n; i++)
            {
                ...
                else if (VALUE_ISA_CLASS(v))
                    vm_set_superclass(vm, (gravity_object_t *)VALUE_AS_CLASS(v));
            }
        }
    }
    else
    ...
    if (vm->filter)
    {
        ...
        while (obj)
        {
            ...
            if (OBJECT_ISA_INSTANCE(obj))
                gravity_instance_deinit(vm, (gravity_instance_t *)obj);
